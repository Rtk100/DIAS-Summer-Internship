...
int main() 
{
    ...
    //Initialise the recordings arrays
    matrix X_vector_recordings[simulation_repetitions/record_steps]; 
    matrix V_vector_recordings[simulation_repetitions/record_steps];
    matrix A_vector_recordings[simulation_repetitions/record_steps]; 
...
// For testing reproducibility use these X values
    ...
    for (int j = 0; j < seconds_thermalised / delta_t; ++j)
    {
        if (j % record_steps == 0)
        {
            ...
            for (int i = 0; i < dim ; i++)
            {
                X_vector_recordings[j/record_steps + i] = X_vector[i];
            }

            for (int i = 0; i < dim ; i++)
            {
                V_vector_recordings[j/record_steps + i] = V_vector[i];
            }

            for (int i = 0; i < dim ; i++)
            {
                A_vector_recordings[j/record_steps + i] << A_vector[i];
            }
    ...
    //Print to text file
    for (matrix Matrix : X_vector_recordings)
        {
    ...
    for (matrix Matrix : V_vector_recordings)
        {
    ...
    // Print to text file
    for (matrix Matrix : A_vector_recordings)
    {
